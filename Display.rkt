...
;          [else (beside (grid-square-place (vector-ref ships-row start-no) w) (show-row ships-row (+ 1 start-no)))]))
  (show-grid 0 oth-pl-strikes w))
...
(define (screen state)
  ...
  (define pl1-grid (cond [(equal? (send state get-mode) 2) (play-grid (get-field strikes-grid-2 state) 0 w)]
                         [(= (send state get-player) 1) (place-grid (convert-to-grid (send state get-sv1)) w)]
                         [else (place-grid (build-grid 10 10 0) w)]))
  (define pl2-grid (cond [(equal? (send state get-mode) 2) (play-grid (get-field strikes-grid-1 state) 0 w)]
