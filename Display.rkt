...
(define (screen state)
  ...
  (define bckg (if (= (send state get-mode) 0)
                   (place-images (list (text "WELCOME TO BATTLESHIPS!!" (exact-floor (* 0.1 h)) "Black")
                                       (text "Choose Mode" (exact-floor (* 0.1 h)) "Black")
                                       (text "One-player Easy" (exact-floor (* 0.09 h)) "Black")
                                       (text "One-player Difficult" (exact-floor (* 0.09 h)) "Black")
                                       (text "Two-player" (exact-floor (* 0.09 h)) "Black")
                                       (rectangle (* 0.5 w) (* 0.1 h) 'solid "lightblue")
                                       (rectangle (* 0.5 w) (* 0.1 h) 'solid "lightblue")
                                       (rectangle (* 0.5 w) (* 0.1 h) 'solid "lightblue"))
                                 (list (make-posn (* 0.5 w) (* 0.1625 h))
                                       (make-posn (* 0.5 w) (* 0.4125 h))
                                       (make-posn (* 0.5 w) (* 0.57 h))
                                       (make-posn (* 0.5 w) (* 0.715 h))
                                       (make-posn (* 0.5 w) (* 0.86 h))
                                       (make-posn (* 0.5 w) (* 0.57 h))
                                       (make-posn (* 0.5 w) (* 0.715 h))
                                       (make-posn (* 0.5 w) (* 0.86 h)))
